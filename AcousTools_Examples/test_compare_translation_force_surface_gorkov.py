...
surface_diameter = diameter + 5 * wavelength
...
for i in range(N+1):
    ...
    Fx,Fy,Fz = compute_force(x, point, board=board, return_components=True, V=volume)
    ...
    f = force_mesh_surface(x, sphere, board, H=H_step, path=sphere_path,
                                       surface=surface, E=E_step, Ex=Ax_step, Ey=Ay_step, Ez=Az_step, sum_elements=True, return_momentum=False, use_momentum=True,
                                       diameter=surface_diameter).real.squeeze()
    
    # f = (f-momentum).sum(dim=2).real.squeeze()
...
# plt.plot([torch.abs(i) for i in pARFzs],c='b', label='$F_z$',linestyle='--')

if __name__ == '__main__':
    ...
    def check_k(x):
        return torch.sqrt(x) / k


    Visualise(*ABC(0.01), x, points=internal_points,
              link_ax=None, titles=['$p$', '$\Delta p$', "$k_{est}/k$" ],
              colour_functions=[propagate_BEM_pressure, propagate_BEM_laplacian_abs, '/'], 
              colour_function_args=[{"path":path, 'H':H, 'board':board, 'scatterer':scatterer, 'p_ref':p_ref}, {"path":path, 'H':H, 'board':board,  'scatterer':scatterer , 'p_ref':p_ref},{"post_function":check_k}],)

...
from acoustools.BEM import force_mesh_surface_curl
...
for i in range(points.shape[2]):
    ...
    curl = force_mesh_surface_curl(x, sphere, board, path=path)

    print(p,curl)

    curls.append(curl.detach().cpu().squeeze(1))


curls = torch.stack(curls, dim=2)

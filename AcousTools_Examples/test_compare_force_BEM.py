...
for i in range(N):
    p = create_points(1,1)
    E = compute_E(sphere, p, board, H=H, path=path)
    x = wgs(p, board=board)
    # x = add_lev_sig(x)
    points.append([p,x])
...
for p,x in points:
    start_fd = time.time()
    F = force_fin_diff(x,p, U_function=BEM_gorkov_analytical, U_fun_args={"path":path,"H":H,"scatterer":sphere})
    end_fd = time.time()
    Fx_fds.append(F[:,0].cpu().detach().item())
    Fy_fds.append(F[:,1].cpu().detach().item())
    Fz_fds.append(F[:,2].cpu().detach().item())
...
for p,x in points:
    start_a = time.time()
    Fx,Fy,Fz = BEM_compute_force(x,p, return_components=True, scatterer=sphere, path=path,H=H)
    end_a = time.time()

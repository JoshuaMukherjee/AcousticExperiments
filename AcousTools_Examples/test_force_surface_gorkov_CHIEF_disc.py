...
p2 = create_points(1,1,0,0,-0.0005)
pressures = []
...
for i in range(N):
    ...
    oct_pth =  path+"/disc_12mm_lam4.stl"
    octa = load_scatterer(oct_pth) #Make mesh at 0,0,0
    scale_to_diameter(octa,d)
    centre_scatterer(octa)
    com = get_centre_of_mass_as_points(octa)
    ...
    internal_points  = get_CHIEF_points(octa, P = 100, start='surface', method='uniform', scale = 0.001, scale_mode='diameter-scale')

    # CHIEF_N = 2
    # internal_points = torch.cartesian_prod(torch.linspace(-0.9*d, 0.9*d, CHIEF_N), torch.linspace(-0.9*d, 0.9*d, CHIEF_N), torch.zeros((CHIEF_N))).T.unsqueeze_(0)
    ...
    E,F,G,H = compute_E(octa, com ,board=board, path=path, use_cache_H=False, p_ref=p_ref,H_method="LU", return_components=True, internal_points=internal_points)
    ...
    A_force= force_mesh_surface(x, octa, board, return_components=False,H=H,path=path,
                                                        diameter=dim, use_cache_H=cache, p_ref=p_ref,internal_points=internal_points).squeeze().detach()
    
    pressure = propagate_BEM_pressure(x, p2, octa, board=board, H=H, path=path, p_ref=p_ref, internal_points=internal_points)

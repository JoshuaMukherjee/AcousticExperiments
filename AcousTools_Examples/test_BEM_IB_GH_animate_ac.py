if __name__ == '__main__':
    ...
    from acoustools.Mesh import load_multiple_scatterers,scale_to_diameter, centre_scatterer, get_edge_data, get_centres_as_points, load_scatterer, merge_scatterers, get_centre_of_mass_as_points
    ...
    def GH_prop(activations, points, scatterer, board, path,use_cache_H, p_ref, H,a,c):
        E,F,G,H = compute_E(scatterer, points,board=board, path=path, use_cache_H=use_cache_H, p_ref=p_ref, return_components=True, H=H,a=a,c=c)


        pressures =  torch.abs(G@H@activations)
        return pressures

    def GH_prop_phase(activations, points, scatterer, board, path,use_cache_H, p_ref, H,a,c):
        E,F,G,H = compute_E(scatterer, points,board=board, path=path, use_cache_H=use_cache_H, p_ref=p_ref, return_components=True, H=H,a=a,c=c)
    ...
    for i,paths in enumerate(os.listdir("/Users/joshuamukherjee/Desktop/Education/University/UCL/PhD/BEMMedia/BEMTest")):
        ...
        H_method = 'OLS'
        E,F,G,H = compute_E(scatterer, p,board=board, path=path, use_cache_H=cache_H, p_ref=p_ref,H_method=H_method, return_components=True, a=a,c=c)

        E_img = Visualise_single(*abc, x, colour_function=propagate_BEM_pressure, colour_function_args={'scatterer':scatterer,'board':board,'path':path,"use_cache_H":cache_H,"p_ref":p_ref, "H":H, "a":a, "c":c } , res=res)
        E_phase = Visualise_single(*abc, x, colour_function=propagate_BEM_phase, colour_function_args={'scatterer':scatterer,'board':board,'path':path,"use_cache_H":cache_H,"p_ref":p_ref, "H":H, "a":a, "c":c  } , res=res)

        GH_img = Visualise_single(*abc, x, colour_function=GH_prop, colour_function_args={'scatterer':scatterer,'board':board,'path':path,"use_cache_H":cache_H,"p_ref":p_ref, "H":H, "a":a, "c":c  } , res=res)
        GH_phase = Visualise_single(*abc, x, colour_function=GH_prop_phase, colour_function_args={'scatterer':scatterer,'board':board,'path':path,"use_cache_H":cache_H,"p_ref":p_ref, "H":H, "a":a, "c":c  } , res=res)
...
for i,frame in enumerate(frames):
    ...
    plt.savefig(f"AcousTools_Examples/outputs/BEM_IB_GH_animate_ac/Diameter{frame[-1]}ac.png", dpi=1000)
    # plt.show()
...
from acoustools.BEM.Gorkov import BEM_gorkov_analytical
import torch
...
board = TOP_BOARD
U_target = torch.tensor([-7.5e-9,]).to(device).to(DTYPE)

p = create_points(1,1,0,0,0.03)

x = gorkov_target(p,board=board, U_targets=U_target, reflector=reflector, path=root)

U = BEM_gorkov_analytical(x, p, reflector, board=board, path=root)
print(U.item())

abc = ABC(0.05)
Visualise(*abc, x, points=p, colour_functions=[propagate_BEM_pressure], colour_function_args=[{'scatterer':reflector,'path':root}])
...
from acoustools.Constants import wavelength, pi, P_ref as PREF, k
...
diameters = torch.linspace(0.0001, 2*wavelength, N)
...
for i in range(N):
    ...
    E,F,G,H = compute_E(sphere, com ,board=board, path=path, use_cache_H=False, p_ref=p_ref,H_method="LU", return_components=True, h=1e-3, BM_alpha=1j/(20*k))
    ...
    pressure = propagate_BEM_pressure(x, p2, sphere, board=board, H=H, path=path, p_ref=p_ref, h=1e-3, BM_alpha=1j/(20*k))
    ...
    E,F,G,Hres = compute_E(sphere, com ,board=board, path=path, use_cache_H=False, p_ref=p_ref,H_method="LU", return_components=True)
    res_force= force_mesh_surface(x, sphere, board, return_components=False,H=Hres,path=path,
                                                        diameter=dim, use_cache_H=cache, p_ref=p_ref).squeeze().detach()

    res_forces_z.append(res_force[2].cpu().detach())
...
plt.plot(rs, res_forces_z, color='b', label='$F_z$', linestyle='--')

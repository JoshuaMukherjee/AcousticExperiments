...
from acoustools.Mesh import load_scatterer, scatterer_file_name, get_edge_data
...
ball_paths = "Media/sphere-lam2.stl"
ball = load_scatterer(ball_paths, dy=-0.06, dz=-0.05) #Make mesh at 0,0,0
# print(ball)
ball.filename = scatterer_file_name(ball)
# print(walls)
get_edge_data(ball)

H = get_cache_or_compute_H(ball,TRANSDUCERS)
...
if plot_path:
    ...
    for i,p in enumerate(points):
        # print(i,end='\r')
        E = compute_E(ball, p, TRANSDUCERS,H=H)
    ...
    def traverse_holo(index):
        # img_ax.clear()
        print(index,end='\r')
        img = Visualise_single_blocks(*abc,holos[index], res=(500,500), colour_function=propagate_BEM_pressure, colour_function_args={'scatterer':ball, 'H':H, 'board':TRANSDUCERS}).cpu().detach().squeeze()
    ...
    ani.save("OptiSpline_Holos_Ball.gif", writer='imagemagick', dpi = 200)
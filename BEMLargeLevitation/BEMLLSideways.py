...
if __name__ == "__main__":
    ...
    print(torch.sum((force_x)).item(), params["weight"], (torch.sum((force_x)) + params["weight"]).item(), 1000 * (torch.sum((force_x)) + params["weight"]).item()/9.81)
    print(torch.sum(torch.abs(force_x)).item(), torch.sum(force_x).item()+ params["weight"], torch.sum(torque_x).item())
    print(torch.sum(torch.abs(force_y)).item(), torch.sum(force_y).item(), torch.sum(torque_y).item())
    print(torch.sum(torch.abs(force_z)).item(), torch.sum(force_z).item(), torch.sum(torque_z).item())

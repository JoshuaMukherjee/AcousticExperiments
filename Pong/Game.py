...
class game():
    ...
    def __init__(self, bound_x = 0.06, bound_z = 0.06, vel_scale=1000):
        self.player_one = create_points(1,1, -1 * bound_x, 0 , 0)
        self.player_two = create_points(1,1,  bound_x, 0 , 0)
        self.ball = create_points(1,1,0,0,0)

        self.paddle_width = 0.0025
        ...
        self.ball_velocity = create_points(1,1,y=0,z=bound_z/4)
        
        self.ball_velocity /= torch.sqrt(torch.sum(torch.square(self.ball_velocity)))
        self.ball_velocity /= vel_scale
